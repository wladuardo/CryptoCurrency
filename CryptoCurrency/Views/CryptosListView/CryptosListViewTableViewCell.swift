...
//  CryptoCurrency
//
//  Created by Владислав Ковальский on 10.06.2022.
//

import UIKit

class CryptosListTableViewCell: UITableViewCell {
    
    static let identifier = "CryptosListViewCustomCell"
    
    private let favoriteCryptosDataService = FavoriteCryptosDataService()
    
    private let cryptosListVc = CryptosListTableViewController()
    ...
    public func configureCellStackView(with viewModel: CryptoTableViewCellModel) {
        cryptoLabel.text = viewModel.name
        cryptoSubLabel.text = viewModel.symbol
        cryptoPriceLabel.text = viewModel.price
    }
    
    public func configureCrytoImageView(with viewModel: CryptoTableViewCellModel) {
